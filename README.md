...
Turbine is an advanced derivate of [Proton](https://github.com/alexbilbie/Proton) and part of 0xF10 (ZeroXF10) Component collection by Marco Bunge.

## Install

### Using Composer

Turbine is available on [Packagist](https://packagist.org/packages/0xF10/Turbine) and can be installed using [Composer](https://getcomposer.org/). This can be done by running the following command or by updating your `composer.json` file.

```bash
composer require 0xF10/Turbine
```
...
This project is also available for download as a `.zip` file on GitHub. Visit the [releases page](https://github.com/0xF10\Turbine/releases), select the version you want, and click the "Source code (zip)" download button.
...
0xF10 Turbine use scotch box with additional xdebug support, modified php setup and mailcatcher for development.

Please read <a href="https://box.scotch.io/" target="_blank">scotch box documentation</a> for more information.

#### Project development

If your project ist depending on `0xF10/Turbine`, you may use the box your project development. You
just need to install your composer dependencies and run following commands within your project 
folder.

```
cp ./vendor/0xF10/Turbine/public .
cp ./vendor/0xF10/Turbine/Vagrantfile .
cp ./vendor/0xF10/Turbine/provision .
```
...
$app = new \ZeroXF10\Turbine\Application();
```
...
$app = new \ZeroXF10\Turbine\Application($config);
```
...
/** @var ZeroXF10\Turbine\Application $app */
...
ZeroXF10\Turbine middlewares allows advanced control of lifecycle execution.
 
ZeroXF10\Turbine uses `ZeroXF10\Turbine\ApplicationRunnerMiddeware` by default, if no middleware with interface 
...
$app = new \ZeroXF10\Turbine\Application(include 'config.php');
$app->addMiddleware(new \ZeroXF10\Turbine\Application\ServiceProvidersFromConfigMiddleware());
```
...
0xF10 Turbine is using routing integration of `league/route` and allows access to route collection methods directly.
...
0xF10 Turbine allows to access `ZeroXF10\Turbine\Application` from anonymous function through closure binding.
...
$app->get('/hello/{name}', function ($request, $response, $args) {
    ...
    // access ZeroXF10\Turbine\Application
    $app = $this;
...
0xF10 Turbine add support for route groups. 
...
- `$this` - `\ZeroXF10\Turbine\Application`
...
$httpKernel = new ZeroXF10\Turbine\Symfony\HttpKernelAdapter($app);
...
0xF10 Turbine uses <a href="https://github.com/filp/whoops" target="_blank">Whoops</a> error handling framework and determines the error handler by request content type.
...
By default 0xF10 Turbine runs with error options disabled. To enable debugging add
...
By default 0xF10 Turbine is catching all errors. To disable error catching add
...
0xF10 Turbine has built in support for Monolog. To access a channel call:
...
ErrorResponse via `ZeroXF10\Turbine\ApplicationEvent`.
...
/** @var \ZeroXF10\Turbine\Application\ApplicationEvent $event */
...
$app->addListener($app::EVENT_REQUEST_RECEIVED, function (\ZeroXF10\Turbine\Application\ApplicationEvent $event) {
...
$app->addListener($app::EVENT_RESPONSE_CREATED, function (\ZeroXF10\Turbine\Application\ApplicationEvent $event) {
    $request = $event->getRequest();
...
$app->addListener($app::EVENT_RESPONSE_SENT, function (\ZeroXF10\Turbine\Application\ApplicationEvent $event) {
    $request = $event->getRequest();
    $response = $event->getResponse();
...
$app->addListener($app::EVENT_RUNTIME_ERROR, function (\ZeroXF10\Turbine\Application\ApplicationEvent $event, $exception) use ($app) {
    //process exception
});
```
...
$app->addListener($app::EVENT_LIFECYCLE_ERROR, function (\ZeroXF10\Turbine\Application\ApplicationEvent $event, \Exception $exception) {
...
$app->addListener($app::EVENT_LIFECYCLE_COMPLETE, function (\ZeroXF10\Turbine\Application\ApplicationEvent $event) {
...
$app->addListener($app::EVENT_SHUTDOWN, function (\ZeroXF10\Turbine\Application\ApplicationEvent $event, $response, $terminatedOutputBuffers = []) {
    // access the response using $event->getResponse()
...
// addListener
$app->addListener('custom.event', function (\ZeroXF10\Turbine\Application\ApplicationEvent $event, $time) {
...
Service providers can be registered using the `register` method on the 0xF10 Turbine app or `addServiceProvider` on the container:
...
0xF10 Turbine uses dependecy injection container to access it's services. Following integrations can be exchanged.
...
- [All contributors](https://github.com/0xF10/Turbine/graphs/contributors)

## License

The MIT License (MIT). Please see [License File](LICENSE.md) for more information.

[ico-version]: https://img.shields.io/packagist/v/0xF10/Turbine.svg?style=flat-square
[ico-license]: https://img.shields.io/badge/license-MIT-brightgreen.svg?style=flat-square
[ico-travis]: https://img.shields.io/travis/0xF10/Turbine/master.svg?style=flat-square
[ico-downloads]: https://img.shields.io/packagist/dt/0xF10/Turbine.svg?style=flat-square
[ico-coveralls]: https://img.shields.io/coveralls/0xF10/Turbine/master.svg?style=flat-square)](https://coveralls.io/github/0xF10/Turbine?branch=1.0.x-dev

[link-packagist]: https://packagist.org/packages/0xF10/Turbine
[link-travis]: https://travis-ci.org/0xF10/Turbine
[link-downloads]: https://packagist.org/packages/0xF10/Turbine
[link-author]: https://github.com/mbunge
[link-contributors]: ../../contributors
[link-coveralls]: https://coveralls.io/github/0xF10/Turbine
